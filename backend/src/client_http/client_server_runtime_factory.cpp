...
bool sameCoord(const pathfinder::world_runtime::WorldCellCoord& a,
               const pathfinder::world_runtime::WorldCellCoord& b) {
    return a.layer_key == b.layer_key && a.x == b.x && a.y == b.y;
}

bool moveFollowersTowardLeader(pathfinder::world_runtime::WorldGridRuntime& runtime,
    ...
    for (const auto& follower_key : followingActorKeys()) {
        ...
        if (sameOrAdjacent8(follower.coord, leader.coord)) continue;
        ...
        std::sort(candidates.begin(), candidates.end(), [&](const auto& left, const auto& right) {
            return manhattan(left, leader.coord) < manhattan(right, leader.coord);
        });

        for (const auto& candidate : candidates) {
            if (sameCoord(candidate, leader.coord)) continue;
...
public:
    ...
    pathfinder::foundation::Result<pathfinder::world_command::WorldCommandExecutionResult> execute(
        ...
        if (command.command_key == "set_actor_follow_player") {
            ...
            if (follower_actor_key.empty() || follower_actor_key == command.actor_key ||
                ...
                !sameOrAdjacent8(actor_res.value()->coord, follower_res.value()->coord)) {
...
ClientServerRuntimeFactory::ClientServerRuntimeFactory()
    ...
    command_gateway.setPostCommandHook([this](const WorldCommandDto& command, ClientCommandResponse& response) {
        const auto run_followers = [&]() {
            if (!world_runtime || command.actor_key.empty()) return;
            (void)moveFollowersTowardLeader(*world_runtime, command.actor_key, response);
        };
