...
void EventLogPanel::setEvents(const std::vector<std::string>& events, const std::string& error) {
    ...
    auto* title = panelLabel(error.empty() ? "事件" : ("阻止：" + error), 16.0F, ax::Vec2(kWidth - 24.0F, 22.0F));
    ...
    for (auto it = events.rbegin(); it != events.rend() && shown < 4; ++it, ++shown) {
        auto* line = panelLabel(shorten(*it, 95), 14.0F, ax::Vec2(kWidth - 24.0F, 20.0F));
