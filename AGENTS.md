...
- 后端内容扩展必须与引擎核心分层，不能把具体玩法内容、内容包或专属规则写进核心服务。
- 物品、资源、反应、生态、任务、区域事件和内容包应优先通过内容定义、配置或注册机制进入系统。
- 需要后端 adapter 或注册代码时，只能依赖 `backend/` 暴露的通用接口，不能复制或绕过通用架构。
- 基础引擎应保持干净、可编译、可测试；删除或替换某个内容包不应破坏核心引擎。
- 前端资源不受后端内容分层约束，但前端仍必须遵守“不写规则、不绕过 Command/available_commands”。

## 当前边界

- 内容包目录规范和自动加载能力需要由对应阶段单独设计，不在入口规则里指定固定目录。
- 不得把“测试里手动构造 `ContentRegistry`”误判为“内容包已经自动加载”。
- 如果要实现“把内容配置文件放进某个目录就自动生效”，必须单独设计 content package loader，并补充加载、校验、冲突检测、版本兼容和回归测试。
